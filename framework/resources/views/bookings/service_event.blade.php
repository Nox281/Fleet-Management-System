<table class="table table-striped">
	...
	<tr>
		...
		<td>
			@if($service->last_meter == 0)
             {{$service->vehicle->int_mileage + $service->services->overdue_meter}} {{ayoubgr::get('dis_format')}}
	        @else
	            {{$service->last_meter + $service->services->overdue_meter}} {{ayoubgr::get('dis_format')}}
	        @endif
		</td>
	</tr>
	<tr>
		...
		<td>
			@php($interval = substr($service->services->overdue_unit,0,-3))
              ...
              @if($service->last_date != 'N/D')
                ...
                @endif
				{{ date(ayoubgr::get('date_format'),strtotime($date)) }}
		</td>
	</tr>
	<tr>
		<th> @lang('fleet.start_date') / @lang('fleet.last_performed')</th>
		<td>
          @lang('fleet.start_date'): {{date(ayoubgr::get('date_format'),strtotime($service->last_date))}}
          <br>
	...
	<tr>
		...
		<td>
			...
			@lang('fleet.or') {{$service->services->overdue_meter}} {{ayoubgr::get('dis_format')}}
			@endif
		</td>
	</tr>
</table>