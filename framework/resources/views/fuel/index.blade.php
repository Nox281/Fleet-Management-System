        ...
        <table class="table" id="data_table">
            ...
            <tr>
              ...
              <td>
                <a href="{{ url("admin/vehicles/".$row->vehicle_id."/edit")}}">
                  ...
                  {{$row->vehicle_data->maker['make']}}-{{$row->vehicle_data->vehiclemodel['model']}}
                </a>
                <br>
                <b>@lang('fleet.vin'):</b> {{$row->vehicle_data['vin']}}
              </td>
              <td>{{date($date_format_setting,strtotime($row->date))}}</td>
              <td> {{$row->qty}} @if(ayoubgr::get('fuel_unit') == "gallon") @lang('fleet.gal') @else Liter @endif </td>
              <td>
                @php ($total = $row->qty * $row->cost_per_unit)
                {{ $currency }} {{$total}}
                <br>
                {{ $currency }} {{$row->cost_per_unit}}/ {{ ayoubgr::get('fuel_unit') }}
              </td>
              <td>
                @lang('fleet.start'): {{$row->start_meter}} {{ayoubgr::get('dis_format')}}
                <br>
                @lang('fleet.end'): {{$row->end_meter}} {{ayoubgr::get('dis_format')}}
                <br>
                @lang('fleet.distence'):

                @if($row->end_meter == 0)
                0.00 {{ayoubgr::get('dis_format')}}
                @else
                {{$row->end_meter - $row->start_meter}} {{ayoubgr::get('dis_format')}}
                @endif
              </td>
              <td>
                {{ $row->consumption }}
          ...
          <tfoot>
            <tr>
              <th>
                @if($data->count() > 0)
                @can('Fuel delete')<button class="btn btn-danger" id="bulk_delete" data-toggle="modal" data-target="#bulkModal" disabled title="@lang('fleet.delete')"><i class="fa fa-trash"></i></button>@endcan
                @endif
              </th>
              <th></th>
              <th></th>
