...
<table>
	...
	<tr><td>Mileage: </td><td>{{$booking->mileage}} {{ ayoubgr::get('dis_format') }}</td></tr>
	<tr><td>Waiting time (in minutes): </td><td>{{$booking->waiting_time}}</td></tr>
	<tr><td>Amount: </td><td>{{ ayoubgr::get('currency') }} {{$booking->total}}</td></tr>
	<tr><td>Total Tax (%): </td><td>{{($booking->total_tax_percent) ? $booking->total_tax_percent : 0}} %</td></tr>
	<tr><td>Total Tax Charges: </td><td>{{ ayoubgr::get('currency') }} {{ ($booking->total_tax_charge_rs) ? $booking->total_tax_charge_rs : 0 }}</td></tr>
    <tr><td>Total : </td><td>{{ ayoubgr::get('currency') }} {{ $booking->tax_total }}</td></tr>
</table>
@endcomponent
