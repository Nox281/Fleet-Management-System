  ...
  <div class="col-md-12">
    <div class="card card-info">
      <div class="card-header">
        ...
        &nbsp;
        @can('WorkOrders add')<a href="{{ route('work_order.create')}}" class="btn btn-success" title="@lang('fleet.create_workorder')"><i class="fa fa-plus"></i></a>@endcan</h3>
      </div>
        ...
        <table class="table" id="ajax_data_table">
          ...
          @foreach($data as $row)
            <tr>
              ...
              <td>
                {{$row->vehicle['year']}}
                {{$row->vehicle->maker['make']}} - {{$row->vehicle->vehiclemodel['model']}}
                <br>
              ...
              <td>
                {{ date($date_format_setting,strtotime($row->created_at)) }}
              </td>
              <td>
              ...
              <td> {{ayoubgr::get('currency')}} {{$row->price}}</td>
              <td> {{ayoubgr::get('currency')}} {{ $row->parts->sum('total') }}</td>
              <td> {{ayoubgr::get('currency')}} {{ $row->price + $row->parts->sum('total') }}</td>
              <td>{{ ($row->mechanic_id)?$row->mechanic->name:"" }}</td>
              <td>
          ...
          <tfoot>
            <tr>
              <th>
                @if($data->count() > 0)
                @can('WorkOrders delete')<button class="btn btn-danger" id="bulk_delete" data-toggle="modal" data-target="#bulkModal" disabled title="@lang('fleet.delete')" ><i class="fa fa-trash"></i></button>@endcan
                @endif
              </th>
              <th>@lang('fleet.vehicle')</th>
              <th></th>
  ...
  $(function(){
    ...
    var table = $('#ajax_data_table').DataTable({
         ...
         columns: [
            ...
            {data: 'required_by', name: 'required_by'},
            {data: 'vendor', name: 'vendor.name'},
            {data: 'description', name: 'description'},
            {data: 'price', name: 'price'},
            {data: 'parts_price_total', name: 'parts_price_total'},            
