        ...
        <div class="row">
          ...
          <div class="form-group" style="margin-right: 10px">
            ...
            <select id="vehicle_id" name="vehicle_id" class="form-control vehicles" style="width: 250px">
              ...
              <option value="{{ $vehicle->id }}" @if($vehicle->id == $vehicle_select) selected @endif>{{$vehicle->maker->make}}-{{$vehicle->vehiclemodel->model}}-{{$vehicle->license_plate}}</option>
              @endforeach
            </select>
          </div>
          <button type="submit" class="btn btn-info" style="margin-right: 10px">@lang('fleet.generate_report')</button>
          <button type="submit" formaction="{{url('admin/print-monthly-report')}}" class="btn btn-danger"><i class="fa fa-print"></i> @lang('fleet.print')</button>
        </div>
        {!! Form::close() !!}
      </div>
    </div>
              ...
              <table class="table table-bordered table-striped table-hover myTable">
                ...
                <thead>
                  <tr>
                    ...
                    <td><strong>{{ ayoubgr::get("currency")}}{{ $income_amt-$expense_amt}}</strong></td>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <th scope="row">@lang('fleet.income')</th>
                    <td>{{ ayoubgr::get("currency")}}{{$income_amt}}</td>
                    </tr>
                    <tr>
                    <th scope="row">@lang('fleet.expenses')</th>
                    <td>{{ ayoubgr::get("currency")}}{{$expense_amt}}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="col-md-4">
            ...
            <div class="table-responsive">
              <table class="table table-bordered table-striped table-hover myTable">
                ...
                @php($tot = $tot + ($exp->driver_amount??$exp->amount))
                @endforeach
                <thead>
                  <tr>
                    <th scope="row">@lang('fleet.incomeByCategory')</th>
                    <td><strong>{{ ayoubgr::get("currency")}}{{$tot}}</strong></td>
                  </tr>
                </thead>
                <tbody>
                  @foreach($income_by_cat as $exp)
                  <tr>
                    ...
                    <td>{{ ayoubgr::get("currency")}}{{$exp->driver_amount??$exp->amount}}</td>
                  </tr>
                  @endforeach
                </tbody>
              </table>
              ...
              <table class="table table-bordered table-striped table-hover expTable">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{{ ayoubgr::get("currency")}}{{$exp->expense}}</td>
                  </tr>
                  @endforeach
                </tbody>
              </table>
