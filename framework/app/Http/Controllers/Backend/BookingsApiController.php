...
class BookingsApiController extends Controller
{
        ...
        if (count($errors) > 0) {
            ...
            if ($request->type == "booking") {
                ...
                $details = array(
                    ...
                    'vehicle' => $booking->vehicle->maker->make . " - " . $booking->vehicle->vehiclemodel->model . " - " . $booking->vehicle->license_plate,
                ...
                if ($service->services->overdue_meter != null) {
                    $interval .= " or " . $service->services->overdue_meter . " " . ayoubgr::get('dis_format');
                }

                $details = array(
                    'vehicle' => $service->vehicle->maker->make . " - " . $service->vehicle->vehiclemodel->model . " - " . $service->vehicle->license_plate,
                    'service_item' => $service->services->description,
                    'next_due_meter' => $next_due_meter . " " . ayoubgr::get('dis_format'),
    ...
    public function receipt($id)
    {
        ...
        if ($row->receipt == 1) {
            ...
            $details = array(
                ...
                'icon' => asset('assets/images/' . ayoubgr::get('icon_img')),
                'date' => date('Y-m-d', strtotime($income->booking_income->date)),
                'from_address' => ayoubgr::get('badd1') . ", " . ayoubgr::get('badd2') . ", " . ayoubgr::get('city') . ", " . ayoubgr::get('state') . ", " . ayoubgr::get('country'),
                ...
                'vehicle' => ($row->vehicle_id) ? $row->vehicle->maker->make . " - " . $row->vehicle->vehiclemodel->model . " - " . $row->vehicle->license_plate : "",
                'driver' => ($row->driver_id) ? $row->driver->name : "",
                'mileage' => $row->mileage . " " . ayoubgr::get('dis_format'),
                'waiting_time' => $row->waiting_time,
                'amount' => ayoubgr::get('currency') . " " . $row->total,
                'tax_percent' => (($row->total_tax_percent) ? $row->total_tax_percent : 0) . "%",
                'tax_charges' => ayoubgr::get('currency') . " " . ($row->total_tax_charge_rs) ? $row->total_tax_charge_rs : 0,
                'total_amount' => ayoubgr::get('currency') . " " . ($row->tax_total) ? $row->tax_total : $row->total,
    ...
    public function push_notification($id)
    {
        ...
        foreach ($select1 as $fetch) {
            ...
            $body = __('fleet.customer') . ": " . $booking->customer->name . ", " . __('fleet.pickup') . ": " . date(ayoubgr::get('date_format') . ' g:i A', strtotime($booking->pickup)) . ", " . __('fleet.pickup_addr') . ": " . $booking->pickup_addr . ", " . __('fleet.dropoff_addr') . ": " . $booking->dest_addr;
            $url = url('admin/bookings');

            $array = array(
                'title' => $title ?? "",
                'body' => $body ?? "",
                'img' => url('assets/images/' . ayoubgr::get('icon_img')),
                'url' => $url ?? url('admin/'),
            );
