    ...
    public function fuel()
    {
        ...
        foreach ($fuel as $row) {
            ...
            if (ayoubgr::get('dis_format') == "km") {
                ...
                $consumption_unit = (ayoubgr::get('fuel_unit') == "gallon") ? "KMPG" : "KMPL";
            } else {

                $consumption_unit = (ayoubgr::get('fuel_unit') == "gallon") ? "MPG" : "MPL";
            }

            $details[] = array(
                'id' => $row->id,
                'photo' => $image,
                'vehicle' => $row->vehicle_data->year . " " . $row->vehicle_data->maker->make . "-" . $row->vehiclemodel->vehicle_data->model,
                'vin' => $row->vehicle_data->vin,
                'date' => $row->date,
                'qty_unit' => ayoubgr::get('fuel_unit'),
                'qty' => $row->qty,
                'currency' => ayoubgr::get('currency'),
                'cost_per_unit' => $row->cost_per_unit,
                'total_cost' => $row->qty * $row->cost_per_unit,
                'distance_unit' => ayoubgr::get('dis_format'),
