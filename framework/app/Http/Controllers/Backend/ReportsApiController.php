...
class ReportsApiController extends Controller
{
    ...
    public function vendor(Request $request)
    {
        ...
        if (isset($request->start_date) && isset($request->end_date)) {
            ...
            $results = DB::select(DB::raw("select vendor_id,sum(price) as total from work_orders where deleted_at is null and created_at between '" . $start . "' and '" . $end . "' group by vendor_id"));
        } else {
            $results = DB::select(DB::raw("select vendor_id,sum(price) as total from work_orders where deleted_at is null group by vendor_id"));
        }
        ...
        if (count($errors) > 0) {
            ...
            foreach ($fuel as $row) {
                if (ayoubgr::get('dis_format') == "km") {
                    if (ayoubgr::get('fuel_unit') == "gallon") {
                        $unit = "KMPG";
                    } else {
                        $unit = "KMPL";
                    }
                ...
                $records[] = array(
                    ...
                    'vehicle' => $row->vehicle_data->maker->make . "-" . $row->vehicle_data->vehiclemodel->model . "-" . $row->vehicle_data->license_plate,
                    'start_meter' => $row->start_meter . " " . ayoubgr::get('dis_format'),
                    'end_meter' => $row->end_meter . " " . ayoubgr::get('dis_format'),
                    'consumption' => $row->consumption . " " . $unit,
                    'cost' => ayoubgr::get('currency') . " " . $row->qty * $row->cost_per_unit,
                );
            }
            $data['success'] = "1";
            $data['message'] = "Data fetched!";
        ...
        if (count($errors) > 0) {
            ...
            foreach ($bookings as $row) {
                $records[] = array(
                    ...
                    'amount' => ($row->receipt == 1) ? ayoubgr::get('currency') . " " . round($row->tax_total, 2) : null,
                );
            }
        ...
        if (count($errors) > 0) {
            ...
            foreach ($income as $row) {
                $records[] = array(
                    'day' => $row->day,
                    'date' => date('Y-m-d', strtotime($row->date)),
                    'income' => ayoubgr::get('currency') . " " . $row->Income2,
                    'vehicle' => $row->vehicle->maker->make . '-' . $row->vehicle->vehiclemodel->model . '-' . $row->vehicle->license_plate,
                );
            }
    ...
    public function expense(Request $request)
    {
        ...
        if (count($errors) > 0) {
            ...
            foreach ($expense as $row) {
                ...
                $expense_records[] = array(
                    'vehicle_make' => $row->vehicle->maker->make,
                    'vehicle_model' => $row->vehicle->vehiclemodel->model,
                    'license_plate' => $row->vehicle->license_plate,
                    'expense_type' => $category,
                    'date' => date('Y-m-d', strtotime($row->date)),
                    'amount' => ayoubgr::get('currency') . " " . $row->amount,
                    'note' => $row->comment,
                );
            }
