...
class BackendApiController extends Controller
{
    ...
    public function dashboard($year)
    {
        ...
        $index['currency'] = ayoubgr::get('currency');
    ...
    public function fare_calc(Request $request)
    {
        ...
        if (count($errors) > 0) {
            ...
            if ($day == 1) {
                $base_km = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_base_km');
                $base_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_base_fare');
                $wait_time_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_base_time');
                $std_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_std_fare');

            } elseif ($day == 2) {
                $base_km = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_weekend_base_km');
                $base_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_weekend_base_fare');
                $wait_time_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_weekend_wait_time');
                $std_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_weekend_std_fare');

            } elseif ($day == 3) {
                $base_km = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_night_base_km');
                $base_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_night_base_fare');
                $wait_time_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_night_wait_time');
                $std_fare = ayoubgr::fare(strtolower(str_replace(' ', '', $vehicle->types->vehicletype)) . '_night_std_fare');
            } else {
                $data['success'] = "0";
    ...
    public function login(Request $request)
    {
        ...
        if (Login::attempt(['email' => $email, 'password' => $password])) {
            ...
            if ($user->language != null) {
                ...
                $language = explode('-', ayoubgr::get("language"));
            }
            $modules = array();
            if ($user->user_type == "S" || $user->user_type == "O") {
