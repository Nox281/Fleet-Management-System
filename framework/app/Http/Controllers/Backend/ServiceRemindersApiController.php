    ...
    public function reminders()
    {
        ...
        foreach ($records as $row) {
            ...
            if ($row->services->overdue_meter != null) {
                $interval .= " or " . $row->services->overdue_meter . " " . ayoubgr::get('dis_format');
            }
            $next_meter = null;
            if ($row->services->overdue_meter != null) {
                if ($row->last_meter == 0) {
                    $next_meter = $row->vehicle->int_mileage + $row->services->overdue_meter . " " . ayoubgr::get('dis_format');
                } else {
                    $next_meter = $row->last_meter + $row->services->overdue_meter . " " . ayoubgr::get('dis_format');
                }
            }
            ...
            if ($row->services->overdue_time != null) {
                ...
                $int = ayoubgr::get('time_interval') . "day";
            }
            ...
            $details[] = array(
                ...
                'image' => $image,
                'unit' => $row->vehicle_id,
                'vehicle' => $row->vehicle->year . " " . $row->vehicle->maker->make . " - " . $row->vehicle->vehiclemodel->model,
                'vin' => $row->vehicle->vin,
