...
use App\Model\ayoubgr as Modelayoubgr;
use Exception;
use ayoubgr;
...
class PaymentController extends Controller
{
    ...
    public function paystack($booking_id)
    {
        try {
            ...
            $is_in_testing = strpos(ayoubgr::payment('paystack_secret'), 'test') !== false ? 1 : 0;
            ...
            $fields = [
                ...
                'currency' => $is_in_testing ? 'ZAR' : ayoubgr::payment('currency_code'),
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                "Authorization: Bearer " . ayoubgr::payment('paystack_secret'),
        ...
        try {
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_HTTPHEADER => array(
                    "Authorization: Bearer " . Modelayoubgr::payment('paystack_secret'),
                    "Cache-Control: no-cache",
                ),
            ));
    ...
    public function stripe($booking_id)
    {
        try {
            $booking = Bookings::find($booking_id);
            \Stripe\Stripe::setApiKey(ayoubgr::payment('stripe_secret_key'));
            $methods = ['card'];
            if (ayoubgr::payment('currency_code') == "EUR") {
                $methods = ['card', 'ideal'];
            }
            $session = \Stripe\Checkout\Session::create([
                ...
                'line_items' => [[
                    'name' => 'Booking',
                    'amount' => $booking->tax_total * 100,
                    'currency' => strtolower(ayoubgr::payment('currency_code')),
                    'quantity' => 1,
                ]],
                'payment_intent_data' => [
    ...
    public function razorpay($booking_id)
    {
        try {
            ...
            $api = new Api(ayoubgr::payment('razorpay_key'), ayoubgr::payment('razorpay_secret'));
            $order = $api->order->create(array('receipt' => $receipt_no, 'amount' => $booking->tax_total * 100, 'currency' => ayoubgr::payment('currency_code'), 'payment_capture' => 1));
            // dd($order);
