...
class UsersApi extends Controller
{
    ...
    public function map_api(Request $request)
    {
        ...
        if (count($errors) > 0) {
            ...
            if (User::where('id', $request->user_id)->where('api_token', $request->api_token)->exists()) {
                ...
                // $km_base = ayoubgr::fare($type . '_base_km');
                // $base_fare = ayoubgr::fare($type . '_base_fare');
    ...
    public function user_single_ride_info(Request $request)
    {
        ...
        if ($booking == null) {
            ...
            $data['success'] = 1;
            $data['message'] = "Data Received.";
            $data['data'] = array('rideinfo' => $rideinfo, 'driver_details' => $driver, 'ride_review' => $review, 'fare_breakdown' => array('base_fare' => ayoubgr::fare(strtolower(str_replace(' ', '', $booking->vehicle->types->vehicletype)) . '_base_fare'),
    ...
    public function share_ride_review($id)
    {
        ...
        $data['data'] = array('rideinfo' => array(
            ...
            'amount' => $booking->getMeta('total'),
            'ride_status' => $booking->getMeta('ride_status'),
            'base_fare' => ayoubgr::fare(strtolower(str_replace(' ', '', $booking->vehicle->types->vehicletype)) . '_base_fare'),
