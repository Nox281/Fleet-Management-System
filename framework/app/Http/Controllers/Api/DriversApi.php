...
class DriversApi extends Controller
{
    ...
    public function driver_rides(Request $request)
    {
        ...
        if ($bookings == null) {
            ...
            $c_rides = array();
            $cancel = array();
            if (ayoubgr::get('dis_format') == 'meter') {
                $unit = 'm';
    ...
    public function single_ride_info(Request $request)
    {
        ...
        if ($booking == null) {
            ...
            $data['data'] = array('rideinfo' => $ride_info, 'user_details' => $user_details, 'ride_review' => $ride_reviews,
                'fare_breakdown' => array('base_fare' => ayoubgr::fare(strtolower(str_replace(' ', '', ($booking->vehicle_id != null ? $booking->vehicle->types->vehicletype : ($vehicle_type->vehicletype ?? "")))) . '_base_fare'), 'ride_amount' => $booking->getMeta('tax_total'), 'extra_charges' => 0, 'payment_mode' => 'CASH')); //done
    ...
    public function destination_reached(Request $request)
    {
        ...
        if ($booking == null) {
            ...
            $km_base = ayoubgr::fare(strtolower(str_replace(' ', '', ($booking->vehicle_id != null ? $booking->vehicle->types->vehicletype : ($vehicle_type->vehicletype ?? "")))) . '_base_km');
            if ($request->get('total_kms') <= $km_base) {
                $total_fare = ayoubgr::fare(strtolower(str_replace(' ', '', ($booking->vehicle_id != null ? $booking->vehicle->types->vehicletype : ($vehicle_type->vehicletype ?? "")))) . '_base_fare');

            } else {
                $total_fare = ayoubgr::fare(strtolower(str_replace(' ', '', ($booking->vehicle_id != null ? $booking->vehicle->types->vehicletype : ($vehicle_type->vehicletype ?? "")))) . '_base_fare') + (($request->get('total_kms') - $km_base) * ayoubgr::fare(strtolower(str_replace(' ', '', ($booking->vehicle_id != null ? $booking->vehicle->types->vehicletype : ($vehicle_type->vehicletype ?? "")))) . '_std_fare'));
            }
            // calculate tax charges
            $count = 0;
            if (ayoubgr::get('tax_charge') != "null") {
                $taxes = json_decode(ayoubgr::get('tax_charge'), true);
            ...
            $data['data'] = array('rideinfo' => $rideinfo, 'user_details' => $user_details, 'ride_review' => $reviews, 'fare_breakdown' => array('base_fare' => ayoubgr::fare(strtolower(str_replace(' ', '', ($booking->vehicle_id != null ? $booking->vehicle->types->vehicletype : ($vehicle_type->vehicletype ?? "")))) . '_base_fare'), 'ride_amount' => $booking->getMeta('tax_total'), 'extra_charges' => 0)); //done
        }
        return $data;
    }
    ...
    public function dest_reach_or_ride_complete($id)
    {
        ...
        $data['data'] = array('rideinfo' => array('booking_id' => $booking->id,
            ...
            'fare_breakdown' => array('base_fare' => ayoubgr::fare(strtolower(str_replace(' ', '', ($booking->vehicle_id != null ? $booking->vehicle->types->vehicletype : ($vehicle_type->vehicletype ?? "")))) . '_base_fare'), //done
    ...
    public function get_settings()
    {
        ...
        foreach ($vehicle_types as $vehicle_type) {
            ...
            $vehicle_type_data[] = array('id' => $vehicle_type->id,
                ...
                'base_fare' => ayoubgr::fare($type . '_base_fare'), //done
                'base_km' => ayoubgr::fare($type . '_base_km'),
                'std_fare' => ayoubgr::fare($type . '_std_fare'),
                'base_waiting_time' => ayoubgr::fare($type . '_base_time'),
                'weekend_base_fare' => ayoubgr::fare($type . '_weekend_base_fare'),
                'weekend_base_km' => ayoubgr::fare($type . '_weekend_base_km'),
                'weekend_wait_time' => ayoubgr::fare($type . '_weekend_wait_time'),
                'weekend_std_fare' => ayoubgr::fare($type . '_weekend_std_fare'),
                'night_base_fare' => ayoubgr::fare($type . '_night_base_fare'),
                'night_base_km' => ayoubgr::fare($type . '_night_base_km'),
                'night_wait_time' => ayoubgr::fare($type . '_night_wait_time'),
                'night_std_fare' => ayoubgr::fare($type . '_night_std_fare'),
            );
        ...
        $data['data'] = array('base_fare' => 500,
            ...
            'night_std_fare' => 20,
            'reasons' => $reason,
            'distance_format' => ayoubgr::get('dis_format'),
            'max_trip_time' => ayoubgr::api('max_trip'),
            'currency_symbol' => ayoubgr::get('currency'),
