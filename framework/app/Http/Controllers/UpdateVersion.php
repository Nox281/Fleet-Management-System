...
class UpdateVersion extends Controller
{
    ...
    public function upgrade61(InstalledFileManager $fileManager)
    {
    ...
    public function upgrade603(InstalledFileManager $fileManager)
    {
        Artisan::call('migrate');
    ...
    public function upgrade602(InstalledFileManager $fileManager)
    {
        Artisan::call('migrate');
        $this->upgradeTo602();
        $this->upgradeTo603();
    ...
    public function upgrade601(InstalledFileManager $fileManager)
    {
        Artisan::call('migrate');
        $this->upgradeTo601();
        $this->upgradeTo602();
    ...
    public function upgrade6(InstalledFileManager $fileManager)
    {
        Artisan::call('migrate');
        $fileManager->update();
        $this->add_permission();
    ...
    public function upgrade5(InstalledFileManager $fileManager)
    {
        Artisan::call('migrate');
        Artisan::call('passport:install');
        TwilioSettings::create(['name' => 'sid', 'value' => '']);
        TwilioSettings::create(['name' => 'token', 'value' => '']);
        TwilioSettings::create(['name' => 'from', 'value' => '']);
        TwilioSettings::create(['name' => 'customer_message', 'value' => '']);
        TwilioSettings::create(['name' => 'driver_message', 'value' => '']);
        $fileManager->update();
        return view('laravel_web_installer.finished');
    }
    ...
    public function frontend_settings(Faker $faker)
    {
        ...
        FrontendModel::updateOrCreate(['key_name' => 'admin_approval', 'key_value' => 0]);
    ...
    {
        $oldPath = "public/" . ayoubgr::get('icon_img');
        $newPath = 'assets/images/' . ayoubgr::get('icon_img');
        if (file_exists("public/" . ayoubgr::get('icon_img'))) {
            \File::copy($oldPath, $newPath);
        }

        $oldPath1 = "public/" . ayoubgr::get('logo_img');
        $newPath1 = 'assets/images/' . ayoubgr::get('logo_img');
        if (file_exists("public/" . ayoubgr::get('logo_img'))) {
    ...
    {
        DB::table('chat_settings')->truncate();
        ChatSettingsModel::create(['name' => 'pusher_app_id', 'value' => '']);
        ChatSettingsModel::create(['name' => 'pusher_app_key', 'value' => '']);
        ChatSettingsModel::create(['name' => 'pusher_app_secret', 'value' => '']);
        ChatSettingsModel::create(['name' => 'pusher_app_cluster', 'value' => '']);
    }

    public function upgradeTo602()
    {
        $this->chat_settings();
        PaymentSettings::updateOrCreate(['name' => 'paystack_secret', 'value' => null]);
        FrontendModel::updateOrCreate(['key_name' => 'booking_time', 'key_value' => 1]);
    }

    public function upgradeTo603()
    {

    }

    public function upgradeTo61()
    {

    }

    public function upgradeTo601()
    {
        Settings::updateOrCreate(['name' => 'income_enable_driver'], ['label' => 'Income enable for Driver', 'value' => 0]);
        Settings::updateOrCreate(['name' => 'expense_enable_driver'], ['label' => 'Expense enable for Driver', 'value' => 0]);
        $drivers = User::where('user_type', 'D')->get();
        foreach ($drivers as $driver) {
            $driver->givePermissionTo(['Transactions list', 'Transactions add', 'Transactions edit', 'Transactions delete', 'VehicleInspection add', 'Fuel add', 'Fuel edit', 'Fuel delete', 'Fuel list']);
        }
    }
    ...
    public function vehicle_types()
    {
        ...
        foreach ($vehicle_types as $key) {
            ...
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_base_fare', 'key_value' => ayoubgr::fare('base_fare'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_base_km', 'key_value' => ayoubgr::fare('base_km'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_base_time', 'key_value' => ayoubgr::fare('base_time'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_std_fare', 'key_value' => ayoubgr::fare('std_fare'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_weekend_base_fare', 'key_value' => ayoubgr::fare('weekend_base_fare'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_weekend_base_km', 'key_value' => ayoubgr::fare('weekend_base_km'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_weekend_wait_time', 'key_value' => ayoubgr::fare('weekend_wait_time'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_weekend_std_fare', 'key_value' => ayoubgr::fare('weekend_std_fare'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_night_base_fare', 'key_value' => ayoubgr::fare('night_base_fare'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_night_base_km', 'key_value' => ayoubgr::fare('night_base_km'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_night_wait_time', 'key_value' => ayoubgr::fare('night_wait_time'), 'type_id' => $key->id]);
            FareSettings::updateOrCreate(['key_name' => strtolower(str_replace(" ", "", $key->vehicletype)) . '_night_std_fare', 'key_value' => ayoubgr::fare('night_std_fare'), 'type_id' => $key->id]);
