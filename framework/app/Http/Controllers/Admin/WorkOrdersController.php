    ...
    public function fetch_data(Request $request)
    {
        if ($request->ajax()) {
            ...
            $work_orders = WorkOrders::select('work_orders.*')->whereIn('vehicle_id', $vehicle_ids)->orderBy('id', 'desc')
                ...
                ->with(['vehicle.maker', 'vehicle.vehiclemodel', 'mechanic', 'vendor', 'parts']);
            ...
            return DataTables::eloquent($work_orders)
                ...
                ->addColumn('vehicle', function ($work_order) {
                    return $work_order->vehicle->year . '<br/>' . $work_order->vehicle->maker->make . '-' . $work_order->vehicle->vehiclemodel->model . '<br/>' .
                ...
                ->editColumn('price', function ($work_order) {
                    return ayoubgr::get('currency') . $work_order->price;
                })
                ->editColumn('parts_price_total', function ($work_order) {
                    return ayoubgr::get('currency') . $work_order->parts->sum('total');
                })
                ->editColumn('total', function ($work_order) {
                    return ayoubgr::get('currency') . ($work_order->price + $work_order->parts->sum('total'));
                })
