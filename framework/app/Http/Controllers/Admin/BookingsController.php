...
class BookingsController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($payments)
                ...
                ->editColumn('amount', function ($row) {
                    return ($row->amount) ? ayoubgr::get('currency') . " " . $row->amount : "";
                })
        ...
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($bookings)
                ...
                ->editColumn('tax_total', function ($row) {
                    return ($row->tax_total) ? ayoubgr::get('currency') . " " . $row->tax_total : "";
                })
                ->addColumn('vehicle', function ($row) {
                    $vehicle_type = VehicleTypeModel::find($row->getMeta('vehicle_typeid'));
                    return !empty($row->vehicle_id) ? $row->vehicle->maker->make . '-' . $row->vehicle->vehiclemodel->model . '-' . $row->vehicle->license_plate : ($vehicle_type->displayname) ?? "";
                })
    ...
    public function get_vehicle(Request $request)
    {
        ...
        if ($req_type == "new") {
            ...
            foreach ($d as $ro) {
                ...
                $text = $vhc->maker->make . "-" . $vhc->vehiclemodel->model . "-" . $vhc->license_plate;
    ...
    public function sms_notification($booking_id)
    {
        ...
        $id = ayoubgr::twilio('sid');
        $token = ayoubgr::twilio('token');
        $from = ayoubgr::twilio('from');
        ...
        $pickup_datetime = date(ayoubgr::get('date_format') . " H:i", strtotime($booking->pickup));
        $dropoff_datetime = date(ayoubgr::get('date_format') . " H:i", strtotime($booking->dropoff));
        ...
        $body = str_replace($search, $replace, ayoubgr::twilio("customer_message"));
        ...
        // send sms to drivers
        $driver_body = str_replace($search, $replace, ayoubgr::twilio("driver_message"));
