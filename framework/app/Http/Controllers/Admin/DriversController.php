    ...
    public function fetch_data(Request $request)
    {
        if ($request->ajax()) {
            ...
            $users = User::select('users.*')
                ...
                ->with(['vehicles.maker', 'vehicles.vehiclemodel', 'metas'])->whereUser_type("D")->groupBy('users.id');
            ...
            return DataTables::eloquent($users)
                ...
                ->addColumn('vehicle', function ($user) {
                    // return ($user->vehicle_id != null) ? $user->driver_vehicle->vehicle->maker->make . '-' . $user->driver_vehicle->vehicle->vehiclemodel->model . '-' . $user->driver_vehicle->vehicle->license_plate : '';

                    # below code is using many-to-many relations
                    // dd($user);
                    $vehicles = [];
                    foreach ($user->vehicles as $vehicle) {
                        $vehicles[] = $vehicle->maker->make . '-' . $vehicle->vehiclemodel->model . '-' . $vehicle->license_plate;
                    }
