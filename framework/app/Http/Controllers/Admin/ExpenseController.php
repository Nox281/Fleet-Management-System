...
class ExpenseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data['total'] = Expense::with(['vehicle.maker', 'vehicle.vehiclemodel'])->whereIn('vehicle_id', $vehicle_ids)->whereDate('date', DB::raw('CURDATE()'))->sum('amount');
        $data['today'] = Expense::with(['vehicle.maker', 'vehicle.vehiclemodel'])->whereIn('vehicle_id', $vehicle_ids)->whereDate('date', DB::raw('CURDATE()'))->get();
    ...
    public function expense_records(Request $request)
    {
        ...
        if ($user->user_type == "D") {
            // $v = DriverLogsModel::where('driver_id',Auth::user()->id)->get();
            // $vehicle_ids = $v->pluck('vehicle_id')->toArray();
            // $data['vehicels'] = VehicleModel::with(['maker', 'vehiclemodel','metas'])->whereId($v->pluck('vehicle_id'))->whereIn_service(1)->get();
            $vehicle_ids = auth()->user()->vehicles()->pluck('vehicle_id')->toArray();
            $data['vehicels'] = auth()->user()->vehicles()->with(['maker', 'vehiclemodel'])->whereIn_service(1)->get();
        } else {
            if ($user->group_id == null || $user->user_type == "S") {
