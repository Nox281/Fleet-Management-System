...
class HomeController extends Controller
{
    ...
    public function export_calendar()
    {
        ...
        foreach ($reminders as $r) {
            ...
            if ($r->last_meter == 0) {
                $next_due = $r->vehicle->int_mileage + $r->services->overdue_meter . " " . ayoubgr::get('dis_format');
            } else {
                $next_due = $r->last_meter + $r->services->overdue_meter . " " . ayoubgr::get('dis_format');
            }
            $interval = $r->services->overdue_time . " " . $r->services->overdue_unit;
            if ($r->services->overdue_meter != null) {
                $interval .= $r->services->overdue_meter . " " . ayoubgr::get('dis_format');
            }
