...
class IncomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->user_type == "D") {
            ...
            if ($user->group_id == null || $user->user_type == "S") {
                $data['vehicels'] = VehicleModel::whereIn_service(1)->with(['maker', 'vehiclemodel'])->get();
                $vehicle_ids = $data['vehicels']->pluck('id')->toArray();
            } else {
                $data['vehicels'] = VehicleModel::whereIn_service(1)->with(['maker', 'vehiclemodel'])->where('group_id', $user->group_id)->get();
                $vehicle_ids = $data['vehicels']->pluck('id')->toArray();
            }
        }
        $data['types'] = IncCats::get();
        $income = IncomeModel::with(['vehicle.maker', 'vehicle.vehiclemodel', 'category'])->whereIn('vehicle_id', $vehicle_ids)->whereDate('date', DB::raw('CURDATE()'));
    ...
    public function destroy(Request $request)
    {
        ...
        if ($user->user_type == "D") {
            ...
            if ($user->group_id == null || $user->user_type == "S") {
                $vehicle_ids = VehicleModel::with(['metas', 'maker', 'vehiclemodel'])->whereIn_service(1)->pluck('id')->toArray();
            } else {
                $vehicle_ids = VehicleModel::with(['metas', 'maker', 'vehiclemodel'])->whereIn_service(1)->where('group_id', $user->group_id)->pluck('id')->toArray();
            }
        }
    ...
    public function income_records(Request $request)
    {
        ...
        $data['types'] = IncCats::get();
        $data['today'] = IncomeModel::with(['vehicle.maker', 'vehicle.vehiclemodel', 'category'])->whereIn('vehicle_id', $vehicle_ids)->whereBetween('date', [$request->get('date1'), $request->get('date2')])->get();
