...
class FrontendApiController extends Controller
{
    ...
    public function language()
    {
        ...
        $l = explode('-', ayoubgr::get("language"));
    ...
    {
        $date_format_setting = (ayoubgr::get('date_format')) ? ayoubgr::get('date_format') : 'd-m-Y';
    ...
    {
        return json_decode(ayoubgr::payment('method'));
    }
    ...
    public function company_info()
    {
        ...
        if (ayoubgr::get('date_format') == 'Y-m-d') {
            $date_setting = "YYYY-MM-DD";
        }
        if (ayoubgr::get('date_format') == 'm-d-Y') {
            $date_setting = "MM-DD-YYYY";
        }
        $data['company_logo'] = url('assets/images/' . ayoubgr::get('logo_img'));
        $data['contact_email'] = ayoubgr::frontend('contact_email');
        $data['company_address'] = ayoubgr::get('badd1') . ", " . ayoubgr::get('badd2') . ", " . ayoubgr::get('city') . ", " . ayoubgr::get('state') . ", " . ayoubgr::get('country') . ".";
        $data['company_phone'] = ayoubgr::frontend('contact_phone');
        $data['customer_support'] = ayoubgr::frontend('customer_support');
        $data['about_breif'] = ayoubgr::frontend('about_us');
        $data['faq_link'] = ayoubgr::frontend('faq_link');
        $data['driver_login_url'] = url('admin/login');
        $data['gmap_api_key'] = ayoubgr::api('api_key');
        $data['facebook'] = ayoubgr::frontend('facebook');
        $data['twitter'] = ayoubgr::frontend('twitter');
        $data['instagram'] = ayoubgr::frontend('instagram');
        $data['linkedin'] = ayoubgr::frontend('linkedin');
        $data['cancellation'] = ayoubgr::frontend('cancellation');
        $data['terms'] = ayoubgr::frontend('terms');
        $data['privacy_policy'] = ayoubgr::frontend('privacy_policy');
    ...
    {
        $data['description'] = ayoubgr::frontend('about_description');
        $data['title'] = ayoubgr::frontend('about_title');
        $data['cities'] = ayoubgr::frontend('cities');
        $data['vehicles'] = ayoubgr::frontend('vehicles');
        $team = TeamModel::get();
        $records = array();
    ...
    {
        $data['about_us'] = ayoubgr::frontend('about_us');
        $data['contact_email'] = ayoubgr::frontend('contact_email');
        $data['contact_phone'] = ayoubgr::frontend('contact_phone');
        $data['address'] = ayoubgr::get('badd1') . ", " . ayoubgr::get('badd2') . ", " . ayoubgr::get('city') . ", " . ayoubgr::get('state') . ", " . ayoubgr::get('country') . ".";
        $data['icon'] = url('assets/images/' . ayoubgr::get('icon_img'));
        $data['logo'] = url('assets/images/' . ayoubgr::get('logo_img'));
        $data['facebook'] = ayoubgr::frontend('facebook');
        $data['twitter'] = ayoubgr::frontend('twitter');
        $data['instagram'] = ayoubgr::frontend('instagram');
        $data['linkedin'] = ayoubgr::frontend('linkedin');
    ...
    public function booking_history($id)
    {
        ...
        foreach ($bookings as $booking) {
            ...
            $data[] = array(
                ...
                'created_time' => date('H:i:s', strtotime($booking->created_at)),
                'receipt' => ($booking->receipt) ? $booking->receipt : 0,
                'id' => $booking->id,
                'paid' => ($booking->payment) ? $booking->payment : 0,
            );
    ...
    public function book_now(Request $request)
    {
        ...
        if (count($errors) > 0 || $request->booking_type != 0) {
            ...
            $booking_time = ayoubgr::frontend('booking_time');
            ...
            $this->book_now_notification($book->id, $vehicle_typeid);
    ...
    public function book_later(Request $request)
    {
        ...
        if (count($errors) > 0 || $request->booking_type != 1) {
            ...
            $booking = Bookings::create(['customer_id' => $request->user_id,
                ...
                'travellers' => $request->no_of_persons,
                'note' => $request->note,
                'pickup' => date('Y-m-d', strtotime($request->journey_date)) . " " . date('H:i:s', strtotime($request->journey_time)),
            ]);
